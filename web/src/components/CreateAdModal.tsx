...
export function CreateAdModal() {
  ...
  async function handleCreateAd(e: FormEvent) {
    ...
    try {
      await axios.post(`http://localhost:3333/games/${gameSelectedAd}/ads`, {
        name: data.name,
          ...
          <Dialog.Content className="fixed bg-[#2A2634] py-8 px-10 text-white top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 rounded-lg w-[480px] shadow-lg shadow-black/25">
            ...
            <form onSubmit={handleCreateAd} className="mt-8 flex flex-col gap-4">
              <div className="flex flex-col gap-2">
                ...
                <Select.Root
                  onValueChange={setGameSelectedAd}
                >
                  <Select.Trigger className="bg-zinc-900 inline-flex items-center justify-center py-3 px-4 rounded text-sm placeholder:text-zinc-500 relative">
                  ...
                  >
                    <Select.Content className="bg-zinc-900 w-[400px]   rounded-md shadow">
                      ...
                      {games.map(game => (
                        <Select.Item key={game.id} value={game.id} className="flex items-center text-zinc-500 px-7 py-1 justify-center hover:bg-violet-500 hover:text-white transition-colors relative">
